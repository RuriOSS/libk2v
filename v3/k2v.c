	...
	for (int i = 0; kv[i] != NULL; i++) {
		if (strcmp(kv[i]->key, key) == 0) {
			...
			case K2V_TYPE_INT:
				...
				*(int *)value = kv[i]->data.scalar == NULL ? 0 : atoi((char *)kv[i]->data.scalar);
				return 0;
			case K2V_TYPE_BOOL:
				if (kv[i]->type != K2V_DATA_TYPE_SCALAR) {
					return -1;
				}
				*(bool *)value = (strcmp(kv[i]->data.scalar, "true") == 0) ? true : false;
				return 0;
			case K2V_TYPE_FLOAT:
				if (kv[i]->type != K2V_DATA_TYPE_SCALAR) {
					return -1;
				}
				*(float *)value = atof((char *)kv[i]->data.scalar);
				return 0;
			case K2V_TYPE_LONG:
				if (kv[i]->type != K2V_DATA_TYPE_SCALAR) {
					return -1;
				}
				*(long *)value = atol((char *)kv[i]->data.scalar);
				return 0;
			...
			case K2V_TYPE_FLOAT_ARRAY: {
				...
				float *arr = (float *)malloc(sizeof(float) * (len + 1));
				for (size_t j = 0; j < len; j++) {
					arr[j] = atof((kv[i]->data.array)[j]);
				}
				*(float **)value = arr;
				return (ssize_t)len;
			}
			case K2V_TYPE_LONG_ARRAY: {
				...
				long *arr = (long *)malloc(sizeof(long) * (len + 1));
				for (size_t j = 0; j < len; j++) {
					arr[j] = atol((kv[i]->data.array)[j]);
				}
				*(long **)value = arr;
				return (ssize_t)len;
			}
